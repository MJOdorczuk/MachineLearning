"""
This file is only meant for our own convincience.
We only used it to easily produce plots without having to disrupt the trainer.py
...
def tune_neural_network(
        ...
        batch_size:int = 16,
        lamb: float = 0,
        learning_rates=np.logspace(-1.5,-3, 6),
                ...
                for activation in activations.get_all_activations():
                    ...
                    for neuron_combination in all_num_neurons_combinations:
                            ...
                            for epoch in range(epochs):
                                ...
                                if epoch % 11 == 10:
                                    ...
                                    if np.abs(np.mean(eval_losses[5:]) - np.mean(eval_losses[10:])) < 0.0001:
                                        ...
                                        break
                            lr_plotter_loss.append(best_loss)
                            lr_plotter_acc.append(best_measure)
                        lr_neuron.append(lr_plotter_loss)
                        lr_acc.append(lr_plotter_acc)
                        ...
                        if best_measure > global_best_measure:
                            ...
                            if best_measure >= 1 and best_loss <0.45:
                                returns = {
                    ...
                    if num_layers == 1:
                        for i, li in enumerate(lr_neuron):
                            plt.plot(np.log10(np.logspace(-4,2,20)), li, label = f'BCE for {i+1} neurons')
                            plt.legend()
                            plt.xlabel('Learning rate (log10)')
                            plt.ylabel('BCE')
                        plt.savefig("figures/lr_loss_1_layer_2.pdf", dpi=100)

                        plt.show()
                        for i, li in enumerate(lr_acc):
                            plt.plot(np.log10(np.logspace(-4,2,20)), li, label = f'Accuracy for {i+1} for neurons')
                            plt.legend()
                            plt.xlabel('Learning rate (log10)')
                            plt.ylabel('Accuracy')
                        plt.savefig("figures/lr_acc_1_layer_2.pdf", dpi=100)
                        plt.show()
                    else:
                        all_num_neurons_combinations = permutations([1,2,3,4,5], num_layers)
                        for i, li in enumerate(all_num_neurons_combinations):
                            plt.plot(np.log10(np.logspace(-4,2,20)), lr_neuron[i], label = f'BCE for {li} neurons')
                            if i <= 10:
                                plt.legend()
                            plt.xlabel('Learning rate (log10)')
                            plt.ylabel('BCE')
                        #plt.savefig("figures/lr_loss_2_layers.pdf", dpi=100)

                        plt.show()
                        all_num_neurons_combinations = permutations([1,2,3,4,5], num_layers)
                        for i, li in enumerate(all_num_neurons_combinations):
                            plt.plot(np.log10(np.logspace(-4,2,20)), lr_acc[i], label = f'Accuracy for {li} for neurons')
                            if i <= 10:
                                plt.legend()
                            plt.xlabel('Learning rate (log10)')
                            plt.ylabel('Accuracy')
                        #plt.savefig("figures/lr_acc_2_layers.pdf", dpi=100)
                        plt.show()
